...
import {
  ...
  RejectInvoice,
  ViewInvoiceDocument,
} from "../../../interfaces/invoiceList";

@Component({
  selector: "ngx-invoices-l3",
  templateUrl: "./invoices-l3.component.html",
  styleUrls: ["./invoices-l3.component.scss"],
})
export class InvoicesL3Component implements OnInit, OnDestroy {
  ...
  ngOnInit(): void {
    ...
    if (this.router.url.toLowerCase().includes("invoices-l3")) {
      this.title = "Invoices List for L3 User";
    }
    // if(!this.isadmin){
  ...
  viewInvoice(id: number) {
    ...
    this.router.navigate(["/pages/invoices-l3", 1]);
  }
  approveInvoice(id: number) {
    console.log("Clicked on Approve Invoice", id);
    this.service
      .approveOrRejectInvoice(id, "/v1/invoice/approove_document_invoice/")
      .subscribe(
        (res: ApproveInvoice) => {
  ...
  rejectInvoice(id: number) {
    ...
    this.service
      .approveOrRejectInvoice(id, "/v1/invoice/reject_document_invoice/")
      .subscribe(
        (res: RejectInvoice) => {
          console.log("Data=", res);
          if (res.code === "200") {
