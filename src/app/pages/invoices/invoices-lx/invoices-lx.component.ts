...
export class InvoicesLxComponent implements OnInit, OnDestroy {
  ...
  ngOnInit(): void {
    ...
    if (this.router.url.toLowerCase().includes("invoices-lx")) {
      this.user_level = sessionStorage.getItem("user_level");
      console.log("global_user_level=", this.user_level);
      this.title = "Invoices List for Users of User Level " + this.user_level;
    }
    // if(!this.isadmin){
